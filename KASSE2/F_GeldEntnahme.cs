...
namespace IS_KASSE
{
    public partial class F_GeldEntnahme : Form
    {
        ...
        private void F_GeldEntnahme_Load(object sender, EventArgs e)
        {
            try
            {
                if (myConn.State == ConnectionState.Closed)
                    myConn.Open();
                string EinlageListSQL = "SELECT * FROM ausgabetype";
                ...
                try
                {
                    if (textBox1.Text != "" && textBox2.Text != "")
                    {
                        try
                        {
                            ...
                            cmdInsert.Parameters.AddWithValue("@type", 2);
                            cmdInsert.Parameters.AddWithValue("@aciklama", "Bargeld Entnahme aus der Kasse:" + gv_Type);
                            ...
                            cmdInsert.Parameters.AddWithValue("@Islem", Islem);
                            cmdInsert.Parameters.AddWithValue("@gv_type", gv_Type);
                            cmdInsert.CommandText = "INSERT INTO `kassenbuch`( `datum`, `type`,  `tutar`, `kaynak`, `aciklama`, `hedef`, `medium`, `username`, kassenr, anfangbestand,gv_type) VALUES (@datum, @type, @betrag, @kaynak, @aciklama, @hedef, @medium,@user, @kassenr,@Islem,@gv_type)";
                        ...
                        {
                            MessageBox.Show("Fehler:" + dd.Message);
                        }
                    ...
                    while (a <= 5)
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                string tseError = "INSERT INTO `tseerror`( `herstellererrorcode`, `datum`, Kassenr, TSEClientID, level,`errorcode`, `tse_errormessage` ) VALUES ('" + wormreturn.errorMessage + "'," + tar.unixdate(DateTime.Now) + "," + Program.kasano + ",'" + Program.ClientID + "', 'GeldEntnahme'"+ErrorMeldungArray[0]+","+WormErrors.Wormerror(Convert.ToInt32(ErrorMeldungArray[0]))+" )";
                                MySqlCommand cmdTseErrorCode = new MySqlCommand(tseError, myConn);
                                cmdTseErrorCode.ExecuteNonQuery();
                            }
                            catch (Exception dd)
                ...
                while (a <= 5)
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            string tseError = "INSERT INTO `tseerror`( `herstellererrorcode`, `datum`, Kassenr, TSEClientID, level) VALUES ('" + wormreturn.errorMessage + "'," + tar.unixdate(DateTime.Now) + "," + Program.kasano + ",'" + Program.ClientID + "', 'GeldEntnahme-FinischTransaction')";
                            MySqlCommand cmdTseErrorCode = new MySqlCommand(tseError, myConn);
                            cmdTseErrorCode.ExecuteNonQuery();
                        }
                        catch (Exception dd)
        ...
        private void btnAuszahlung_Click(object sender, EventArgs e)
        {
            Button btnClick = sender as Button;
            if (btnClick.Name == "btnAuszahlung")
            {
                Islem = 24; //4
                gv_Type = "Auszahlung";
            }
            else if (btnClick.Name == "btnLohnzahlung")
            {
                Islem = 22; //6
                gv_Type = "Lohnzahlung";
            }
            else if (btnClick.Name == "btnPrivatentnahme")
            {
                Islem = 19; //2
                gv_Type = "Privatentnahme";
            }
