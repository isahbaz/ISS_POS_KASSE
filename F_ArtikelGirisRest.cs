...
namespace IS_KASSE
{
    public partial class F_ArtikelGirisRest : Form
    {
        ...
        private void F_ArtikelGiris_Load(object sender, EventArgs e)
        {
            ...
            dgvUrun.Columns.Add("bnarkod", "BARKOD");
            dgvUrun.Columns.Add("ad", "ARTİKEL AD");
            dgvUrun.Columns.Add("AlisFiyat", "ALISFiyat");
            dgvUrun.Columns.Add("satisfiyat", "SATISFiyat");
            label13.Text = "";
            ...
            if (hesapislem == false)
            {
                ...
                if (double.TryParse(txtAlisfiyat.Text, out alisfiyat))
                {
                    if (double.TryParse(txtSatisfiyat.Text, out satisfiyat))
                    {
                        ...
                        double kar = (Convert.ToDouble(txtSatisfiyat.Text) - (Convert.ToDouble(txtSatisfiyat.Text) * mwst) / 100) - (alisfiyat * (1 + mwst / 100));
                        txtKarE.Text = kar.ToString();
                        txtKar.Text = ((kar / alisfiyat) * 100).ToString();
                    }
                }
                else
                {
        ...
        private void btnYeni_Click(object sender, EventArgs e)
        {
            ...
            double satisfiyat, satisfiyat2=0, satisfiyat3=0;
            double agirlik;
            double  alisfiyat;
            if (double.TryParse(txtAlisfiyat.Text, out alisfiyat))
            {
            ...
            if (double.TryParse(txtSatisfiyat.Text, out satisfiyat))
            {
                ...
                if (double.TryParse(txtPreis2.Text, out satisfiyat2))
                {
                }
                if (double.TryParse(txtPreis3.Text, out satisfiyat3))
                {
                }
            ...
            if (int.TryParse(txtStok.Text, out stok))
            {
                ...
                else
                {
                    if (eskistok != -1 )
                    {
                        stokSQL = "toplamstok+" + txtStok.Text;
                    }
                    else
                    {
                        stokSQL = txtStok.Text;
            ...
            if ((grupID != -2) && (KayitKontrol() == true))
            {
                ...
                string UpdateSql = "UPDATE artikel SET barkod='" + txtBarkod.Text + "', artikelad='" + txtAd.Text + "', grupid=" + cbbGrup.SelectedValue + ", liferantid=" + cbbLieferant.SelectedValue + ",firmaid=" + cbbMarka.SelectedValue +
                ...
                ", karmiktari=" + vA.virgulayikla(Convert.ToDouble(txtKarE.Text)) + ", toplamstok= " + stokSQL + ", agirlik= " + txtAgirlik.Text + ", birimfiyat=" + vA.virgulayikla(birimfiyat) + ", fand=" + cbbPfand.SelectedValue + ",regal=" + raf + ",sira=" + sira + ", fand2=" + fand2 + 
                ", partnerschaft=" + fand3 +",preis2="+vA.virgulayikla(satisfiyat2)+",preis3="+vA.virgulayikla(satisfiyat3)+ "  WHERE artikelid=" + grupID;
                MySqlCommand coUpdate = new MySqlCommand(UpdateSql, myConn);
                try
                {
                    if (coUpdate.ExecuteNonQuery() > 0)
                    {
                        label13.Text ="Aktualisierung ist OK!";
                        if (rbInventur.Checked == true) //Inventur mu?? evet
                        {
                            ...
                            coSayim.CommandText = "CALL SayimHareket(" + grupID + "," + stok + ", @satisfiyat, @alisfiyat," + cbbLieferant.SelectedValue + ")";
                        ...
                        else //Inventur mu?? hayır
                        {
                            ...
                            {
                                sonstok = stok;
                                comTXT = "CALL SiparisHareket(" + grupID + "," + sonstok + ", @satisfiyat, @alisfiyat," + cbbLieferant.SelectedValue + ",1)";
                            }
                            else if (eskistok != -1 && eskistok != stok)
                            {
                                sonstok = stok;
                                comTXT = "CALL SiparisHareket(" + grupID + "," + sonstok + ", @satisfiyat, @alisfiyat," + cbbLieferant.SelectedValue + ",0)";
                            }
                ...
                {
                    label13.Text ="FEHLER! "+ee.Message;
                }
            ...
            else if ((grupID == -2) && (KayitKontrol() == true))
            {
                ...
                //`artikelid``barkod``artikelad``grupid``liferantid``firmaid``mwst``alisfiyat``satisfiyat``birimid``karmiktari`
                //`toplamstok``artikeltur``angebotdurum``angebotbaslamatarih``angebotbitistarih``angebotfiyat`
                string InsertSql = "INSERT INTO artikel SET barkod='" + txtBarkod.Text + "', artikelad='" + txtAd.Text + "', grupid=" + cbbGrup.SelectedValue + ", liferantid=" + cbbLieferant.SelectedValue + ",firmaid=" + cbbMarka.SelectedValue +
                ...
                ", toplamstok= " + stokSQL + ", agirlik= " + txtAgirlik.Text + ", birimfiyat=" + vA.virgulayikla(birimfiyat) + ", fand=" + cbbPfand.SelectedValue + ",regal=" + raf + ",sira=" + sira + ", fand2=" + fand2 + ", partnerschaft=" + fand3 + ",preis2=" + vA.virgulayikla(satisfiyat2) + ",preis3=" + vA.virgulayikla(satisfiyat3);
                ...
                try
                {
                    if (coUpdate.ExecuteNonQuery() > 0)
                    {
                        ...
                        label13.Text ="Artikel wurde hinzugefügt!";
            ...
            {
                label13.Text="Bitte prüfen Sie Ihre Angabe!";
            }
            barkodno = "";
            grupID = -2;
        }
        ...
        private void txtBarkod_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Return)
            {
                ...
                aramayap(); //GridiDoldur(); // Gridi doldurmak için yazdığın metod
        ...
        private void aramayap()
        {
            ...
            MySqlDataAdapter myDaGrup = new MySqlDataAdapter("SELECT * from artikel where barkod = '" + barkodno + "'", myConn);
            ...
            if (kaySay > 0)
            {
                ...
                txtPreis2.Text = dtGrup.Rows[0].ItemArray[42].ToString();
                txtPreis3.Text = dtGrup.Rows[0].ItemArray[43].ToString();
