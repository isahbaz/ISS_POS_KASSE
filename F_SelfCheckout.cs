...
using iss_tse_v2;
using iss_HandyAuflade_Manegement;
using Digi_ISS;
using ept_extended;

using iss_Artikel;
using iss_HandyAuflade_Manegement_Mopin;
...
namespace IS_KASSE
{
    public partial class F_SelfCheckout : Form
    {
        ...
        public F_SelfCheckout()
        {
            KameraStart();
            Program.bonDruck = true;

            // MessageBox.Show("Kasa Start");
            try
            {
                ...
                {
                    panelFav.Location = new Point(1020, 0);
                    panelFav.Size = new Size(Screen.FromControl(this).Bounds.Width - 1024, 740);
                    panelSetup.Location = new Point(0, 741);
        ...
        private void btnWaageML_Click(object sender, EventArgs e)
        {
            try
            {
                string apppath = Application.StartupPath;
                string resimpath = "" , kisaPath="";
                kisaPath = tarih.unixdate(DateTime.Now) + ".jpg";
                resimpath = apppath + "\\KI\\" +kisaPath;
                //kisaPath = tarih.unixdate(DateTime.Now) + ".jpg";
                ...
                pro.StartInfo.FileName = @"c:\Windows\Sysnative\cmd.exe";
                //pro.StartInfo.WorkingDirectory = Path.GetDirectoryName(apppath + "\\KI");
                pro.StartInfo.CreateNoWindow = true;
                
                pro.StartInfo.Arguments = "/c KI\\ISS_ML_CONSOL.exe KI\\" + kisaPath; // tarih.unixdate(DateTime.Now) + ".JPG";
                //pro.StartInfo.Arguments = args[0];
                pro.Start();

                //Process pro =Process.Start(@"c:\Windows\Sysnative\cmd.exe", "/c KI\\ISS_ML_CONSOL.exe KI\\"+kisaPath);
                //pro.WaitForExit();
                //MessageBox.Show(pro.StartInfo.Arguments);
                string jsonOutput = "";
                
                jsonOutput = pro.StandardOutput.ReadToEnd();
                
                pro.WaitForExit();
                
                int a = jsonOutput.IndexOf('{');
                int uz = jsonOutput.Length;
                String kalan = "";
                //MessageBox.Show(jsonOutput);
                kalan = jsonOutput.Substring(a, uz - a);
                // JSON'u Dictionary formatına çevir
                Dictionary<string, float> data = JsonConvert.DeserializeObject<Dictionary<string, float>>(kalan);
                //MessageBox.Show(data.Count.ToString());
                // Dictionary içeriğini ekrana yaz
                if(data.Count>1)
                {
                    string text = "";
                    ...
                    F_GenericError errForm = new F_GenericError();
                    errForm.lblMesaj.Text = "Bitte wiegen Sie jedes Obst und Gemüse einzeln. Mehrere wurden erkannt!\n" + text;
                    errForm.ShowDialog();
                    return;
                }
                else if(data.Count==1)
                {
                    ...
                    if (connection.State == ConnectionState.Closed)
                        connection.Open();
                    KeyValuePair<string, float> keyValue = new KeyValuePair<string, float>();
                    
                    foreach (KeyValuePair<string, float> item in data)
                    {
                        lblParaUstu.TextAlign = ContentAlignment.MiddleLeft;
                        lblParaUstu.Text = item.Key + "(" + item.Value.ToString("#0.000") + "%)"; ;
                        string SQLWaage = "SELECT * FROM artikel WHERE ai='" + item.Key + "'";
                        MySqlDataAdapter myDaArt = new MySqlDataAdapter(SQLWaage, connection);
                        DataTable dtArt = new DataTable();
                        myDaArt.Fill(dtArt);
                        string urunad = "";
                        long artikelid = -1;
                        string satilanBarkod = dtArt.Rows[0].ItemArray[1].ToString();
                ...
                MLMain mainML = new MLMain();
                mainML.ModelPredict(pictureBox1.Image);
                videoSource.NewFrame += new AForge.Video.NewFrameEventHandler(videoSource_NewFrame1);*/
            }
            catch (Exception dd)
            {
        ...
        private void KameraStart()
        {
            videosources = new FilterInfoCollection(FilterCategory.VideoInputDevice);
            //videosource2 = new FilterInfoCollection(FilterCategory.VideoInputDevice);
            ...
            if (videosources != null)
            {
                ...
                try
                {
                    ...
                    {
                        string highestSolution= "0;0", highestSolution2 = "0;0";
                        //Search for the highest resolution
                        for (int i = 0; i < videoSource.VideoCapabilities.Length; i++)
                        {
                            if (videoSource.VideoCapabilities[i].FrameSize.Width > Convert.ToInt32(highestSolution.Split(';')[0]))
                                highestSolution = "800" + ";" + i.ToString();
                        }
                        //videoSource.VideoCapabilities[1].FrameSize.Height = 100;
                        //Set the highest resolution as active

                        // videoSource.DesiredFrameSize = videoSource.VideoCapabilities[0].FrameSize;
                    }
                    if (videoSource2.VideoCapabilities.Length > 0)
                    {
                        string highestSolution2 = "0;0";
                        //Search for the highest resolution
                        for (int i = 0; i < videoSource2.VideoCapabilities.Length; i++)
                        {
                            if (videoSource2.VideoCapabilities[i].FrameSize.Width > Convert.ToInt32(highestSolution2.Split(';')[0]))
                                highestSolution2 = "800" + ";" + i.ToString();
                        }
                        //videoSource.VideoCapabilities[1].FrameSize.Height = 100;
                        //Set the highest resolution as active
        ...
        {
            panelKapak.Dock = DockStyle.Fill;
                                    ...
                                    foreach (RabattMain rabat in yeniFis.RabatList)
                                    {
                                        if (rabat.Grupid != 999 && rabat.RabatArt != 6)
                                        {
                                            if (rabat.TotalRabattMenge != 0)
                                            {
                                                ...
                                                satisYap.KasaNo = Program.kasano;
                                                satisYap.Mwst = 0;
                                                satisYap.Ustid_id = satisYap.Mwst == Program.MwStList[1] ? ((int)UstIdEnum.mwst7) : (satisYap.Mwst == 19 ? (int)UstIdEnum.mwst19 : (int)UstIdEnum.mwst0);
        ...
        {
            using (MySqlConnection myConn = (new db()).myconn())
            {
        ...
        b:
            ...
            this.Enabled = true;
            panelKapak.BringToFront();
            panelKapak.Visible = true;
        }
                        ...
                        if (int.TryParse(gelenBarkod.Substring(2, 5), out BizerbaBonNr))
                        {
                            if (BizerbaBonNr != 0)
                            {
                                ...
                                using (connection)
                                {
                                    if (connection.State == ConnectionState.Closed)
                                        connection.Open();
                                    string BizSQL = "SELECT  biz_umsatz_pos.*, ABEZ,EK, EAN1, biz_umsatz_kopf.gelesen FROM biz_umsatz_pos LEFT JOIN `biz_artikel` ON PLNR=artnr INNER JOIN biz_umsatz_kopf ON biz_umsatz_kopf.bonnr=biz_umsatz_pos.bonnr WHERE biz_umsatz_pos.bonnr=" + BizerbaBonNr + " AND biz_umsatz_kopf.gelesen=0";
                                    ...
                                    if (dtBiz.Rows.Count > 0)
                                    {
                                        //Read Check
                                        string bizCheck = "";
                                        bizCheck = "SELECT * FROM biz_umsatz_kopf WHERE bonnr=" + BizerbaBonNr + " AND gelesen=0";
                                        ...
                                        if (dtCheck.Rows.Count > 0)
                                        {
                                            ...
                                            {
                                                for (int q = 0; q < dtBiz.Rows.Count; q++)
                                                {
                                                    satisYap = new SatisYap();
                                                    satisYap.Adet = Convert.ToDouble(dtBiz.Rows[q].ItemArray[8]);
                                                    satisYap.Fisno = yeniFis.SatisAnaId;
                                                    satisYap.Barkod = dtBiz.Rows[q].ItemArray[14].ToString();
                                                    satisYap.KasaNo = Program.kasano;
                                                    satisYap.Mwst = Convert.ToDouble(dtBiz.Rows[q].ItemArray[6]);
                                                    satisYap.Ustid_id = satisYap.Mwst == Program.MwStList[1] ? ((int)UstIdEnum.mwst7) : (satisYap.Mwst == Program.MwStList[2] ? (int)UstIdEnum.mwst19 : (int)UstIdEnum.mwst0);
                                                    satisYap.Alisfiyat = dtBiz.Rows[q].ItemArray[13] is DBNull ? 0 : Convert.ToDouble(dtBiz.Rows[q].ItemArray[13]);
                                                    satisYap.Satisfiyat = Convert.ToDouble(dtBiz.Rows[q].ItemArray[7]);
                                                    satisYap.Tarih = (double)tarih.unixdate(DateTime.Now);
                                                    satisYap.UrunId = Convert.ToDecimal(dtBiz.Rows[q].ItemArray[5]);
                                                    satisYap.Birimkar = Convert.ToDouble(dtBiz.Rows[q].ItemArray[7]) - (dtBiz.Rows[q].ItemArray[13] is DBNull ? 0 : Convert.ToDouble(dtBiz.Rows[q].ItemArray[13]));
                                                    ArtikelGrup artGrp = new ArtikelGrup(1);
                                                    artGrp.ArtikelGrupInfo(" WHERE isFleischtheke=1");
                                                    satisYap.Grubid = artGrp.Grupno;
                                                    satisYap.Gruptur = 3;
                                                    satisYap.Toplamtutar = Convert.ToDouble(dtBiz.Rows[q].ItemArray[11]);
                                                    ...
                                                    satisYap.UrunAd = dtBiz.Rows[q].ItemArray[12] is DBNull ? "Fleisch-Produkte" : dtBiz.Rows[q].ItemArray[12].ToString();
                                                    yeniFis.SatisKalem.Add(satisYap);
                                                    int count = listView1.Items.Count;
                                                    listView1.Items.Add(position.ToString());
            ...
            char[] chArray2 = new char[14]
      {
        ...
        private string getMaxID(string Brkd)
        {
            MySqlConnection mySqlConnection = new MySqlConnection();
            MySqlConnection connection;
            using (connection = new db().myconn())
            {
                if (connection.State == ConnectionState.Closed)
                    connection.Open();
            ...
            /*  MySqlConnection mySqlConnection = new MySqlConnection();
              ...
              if (count > 0)
              {
                  ...
                  for (int index = 0; index < count; ++index)
                  {
                      ...
                      kryptonButton.Location = new Point(3, 3);
        ...
        private void geraboSerialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            int num1 = (int)MessageBox.Show(Encoding.ASCII.GetString(numArray));
            List<int> intList = new List<int>();
        ...
        {
            //MLMain main = new MLMain();

        }

        private void panelKapak_Click(object sender, EventArgs e)
        {
            panelKapak.Visible = false;
        }

        private void panelKapak_Paint(object sender, PaintEventArgs e)
        {
